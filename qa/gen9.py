...
const S1 = { title:'Показать квартиру · ур. 55',
  ...
  nodes:{
    place:{ task:'Скажи «да, это маленькая квартира».', best:'Yes, it is a small flat.',
    judge(w){
          if(chose(w,'bye','goodbye','later','leave','leaving')) return {br:'bye'};
          if(chose(w,'like','love','nice','cosy','great')) return {br:'like'};
          if(chose(w,'big','large')) return {br:'big'};
          if(chose(w,'flat','apartment','small','little','new','place','yes','yeah')) return {br:'ok'};
